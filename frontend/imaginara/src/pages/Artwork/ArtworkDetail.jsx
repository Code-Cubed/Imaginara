...
const ArtworkDetail = ({ onLogout }) => {
  ...
  const fetchBookmarkStatus = async () => {
    ...
    try {
      ...
      if (response.ok && data.artworks) {
        const bookmarked = data.artworks.some(item => item._id === id);
  ...
  const fetchSimilarArtworks = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/similarity/artwork/${id}?limit=6`);
      const data = await response.json();

      if (response.ok && data.similar) {
        setSimilarArtworks(data.similar);
      }
    } catch (err) {
      ...
      <div className="detail-container">
        ...
        <div className="artwork-detail">
          ...
          <div className="info-section">
            ...
            {!loadingSimilar && similarArtworks.length > 0 && (
              <div className="similar-artworks-section">
                <h3 className="section-title">
                  <Sparkles size={24} />
                  Similar Artworks
                </h3>
                <div className="similar-artworks-grid">
                  {similarArtworks.map((similar) => (
                    <div
                      key={similar._id}
                      className="similar-artwork-card"
                      onClick={() => navigate(`/artwork/${similar._id}`)}
                    >
                      <div className="similarity-badge-small">
                        {similar.similarityScore}% match
                      </div>
                      <div className="similar-artwork-image">
                        {similar.mediaType === 'image' ? (
                          <img
                            src={similar.thumbnailUrl || similar.mediaUrl}
                            alt={similar.title}
                          />
                        ) : (
                          <div className="similar-placeholder">
                            {similar.mediaType === 'audio' ? '🎵' : '📄'}
                          </div>
                        )}
                      </div>
                      <div className="similar-artwork-info">
                        <p className="similar-artwork-title">{similar.title}</p>
                        <p className="similar-artwork-creator">
                          {similar.creator?.name}
                        </p>
                      </div>
                    </div>
                  ))}
